...
class ReUploadingVQC(nn.Module):
    def __init__(self, n_qubits: int, qnn_layers: int, device_name: str = "default.qubit"):
        super().__init__()
...
class QuantumActor(nn.Module):
    ...
    def _to_simplex(self, y: torch.Tensor) -> torch.Tensor:
        # y: (B, action_dim)
        a = torch.nn.functional.softplus(y)            # a >= 0 (good gradient)
        w = a / (a.sum(dim=1, keepdim=True) + 1e-8)    # w in simplex, sum=1
        return w
    ...
    def sample(self, state):
        ...
        z = normal.rsample()                # (B,2) trong R
        w = self._to_simplex(z)             # map lên simplex
        action = w * self.max_action        # max_action=1.0 → giữ nguyên tỉ lệ
        # log_prob cho biến đổi simplex (softplus+normalize) ta coi như
        # policy trên R rồi transform; thực tế khó đóng form => dùng log_prob của z:
...
class SACAgent:
    def __init__(self, state_dim, action_dim, max_action, device,
        ...
        self.target_entropy = -1.25 *action_dim
    ...
    def select_action(self, state, evaluate=False):
        ...
        if evaluate:
            ...
            action = self.actor._to_simplex(mean) * self.max_action
        else:
...
if __name__ == '__main__':
    ...
    with open("ModelInfo/model_info_QSAC_6qubits_3layers_new.txt", "w", encoding="utf-8") as f:
    ...
    for ep in range(episodes):
        ...
        print(f"Episode {ep}, Reward: {total_reward:.2f}, | Demand=({demand_lte:.1f}, {demand_nr :.1f}) "
              f"| Alloc=({alloc_lte:.1f}, {alloc_nr:.1f})", flush=True)

    # Save & plot
    torch.save(agent.actor.state_dict(), "Model/sac_qactor_actor_model_new.pth")
    pd.DataFrame(logs).to_csv("Result/sac_qactor_training_log_new.csv", index=False)
    ...
    plt.grid(True)
    plt.savefig("FigureReward/sac_qactor_training_reward_plot_new.png")
    plt.show()
