...
CKPTS = {
    "QSAC": {"arch":"qactor_classic_critic", "ckpt":"Model/sac_qactor_actor_model_morePara_desaturation_ver2.pth"},
...
def build_agent(arch, state_dim, action_dim, max_action, device):
    ...
    if arch.startswith("qactor"):
        from SAC_QonlyCritic_desaturation2 import SACAgent
        return SACAgent(state_dim, action_dim, max_action, device)
    elif "sac" in arch:
...
def run_model_timeseries(name, cfg):
    ...
    for step in range(max_steps):
        ...
        if terminated or truncated:
            # Chúng ta mong muốn đủ 0..99; nếu env kết thúc sớm thì dừng tại đó.
            break
    ...
    for m, sub in df.groupby("model"):
        ...
        row = {
            ...
            **{k: float(v) for k, v in ob.items()},
            # (tuỳ chọn) thêm độ lệch chuẩn để tham khảo
...
df.to_csv("outputs/compare_models_timeseries.csv", index=False)
print("[INFO] saved -> outputs/compare_models_timeseries.csv")


# ====== Statistics =====
ts_csv = "outputs/compare_models_timeseries.csv"
metrics_df = compute_metrics_from_timeseries(df, N_R=60.0, eps_list=(0.05,0.10,0.15))
os.makedirs("outputs", exist_ok=True)
metrics_df.to_csv("outputs/compare_models_metrics.csv", index=False)
try:
    metrics_df.to_excel("outputs/compare_models_metrics.xlsx", index=False)
except Exception as e:
    print("[WARN] cannot write xlsx:", e)
print("[INFO] saved metrics -> outputs/compare_models_metrics.(csv/xlsx)")
print(metrics_df)
...
# ---- 1–4: Allocation vs Demand (time-series) cho từng model ----
...
for k, m in enumerate(model_order):
    ...
    ax.set_title(f"{m}: Allocation vs Demand")
    ax.set_xlabel("Step"); ax.set_ylabel("Resource")
    #ax.set_ylim(0, 40)
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best", fontsize=9)

# ---- 5: Surplus/Deficit LTE theo thời gian (4 đường – mỗi model một đường) ----
ax5 = axes[4]
for m in model_order:
    sub = df[df["model"] == m].sort_values("step")
    ax5.plot(sub["step"], sub["surplus_lte"], linestyle="--", label=m)
ax5.set_title("Surplus/Deficit over Time (LTE)")
ax5.set_xlabel("Step"); ax5.set_ylabel("Surplus")
ax5.grid(True, alpha=0.3); ax5.legend()

# ---- 6: Surplus/Deficit NR theo thời gian (4 đường – mỗi model một đường) ----
ax6 = axes[5]
for m in model_order:
    sub = df[df["model"] == m].sort_values("step")
    ax6.plot(sub["step"], sub["surplus_nr"], linestyle="-", label=m)
ax6.set_title("Surplus/Deficit over Time (NR)")
ax6.set_xlabel("Step"); ax6.set_ylabel("Surplus")
ax6.grid(True, alpha=0.3); ax6.legend()

plt.tight_layout()
out_png = "outputs/compare_models_6subplots_timeseries.png"
