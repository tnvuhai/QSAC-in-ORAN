...
# ========= Quantum approach 2 limits =========
def _rand_init_near_pi_over_2(tensor, eps=0.1):
    with torch.no_grad():
        tensor.uniform_(math.pi/2 - eps, math.pi/2 + eps)
...
class ReUploadingVQC(nn.Module):
    def __init__(self, n_qubits: int, qnn_layers: int, device_name: str = "lightning.qubit"):
        ...
        self.phi   = nn.Parameter(torch.zeros(qnn_layers, n_qubits))

        # Pre-engtanglement Rot(α,β,γ) and post_engtanglement Rot(α,β,γ)
        self.rot_pre = nn.Parameter(torch.zeros(n_qubits, 3))
        self.rot_post = nn.Parameter(torch.zeros(n_qubits, 3))
        _rand_init_near_pi_over_2(self.rot_post, eps=0.1)
        ...
        def circuit(inputs, ent_w, omega, phi, rot_pre, rot_post):
    ...
    def forward_single(self, x: torch.Tensor) -> torch.Tensor:
        out = torch.stack(self.circuit(x, self.entangling_weights, self.omega, self.phi, self.rot_pre, self.rot_post))
...
class QuantumActor(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, max_action: float,
                 n_qubits: int = 6, qnn_layers: int = 2, device_name: str = "default.qubit"):
        super().__init__()
    ...
    def forward(self, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        feats = self.vqc(x).to(state.device)                 # giờ feats sẽ là float32
...
class SACAgent:
    def __init__(self, state_dim, action_dim, max_action, device,
                 n_qubits: int = 6, qnn_layers: int = 2, q_device_name: str = "default.qubit",
    ...
    def select_action(self, state, evaluate=False):
        ...
        if evaluate:
            ...
            #print("mean:", mean.detach().cpu().numpy(), "std:", std.detach().cpu().numpy())
    ...
    def train(self, replay_buffer, batch_size=48, gamma=1.0, tau=1e-4):
        ...
        if do_policy:
            ...
            actor_loss.backward()
            torch.nn.utils.clip_grad_norm_(self.actor.parameters(), 1.0)  # (khuyên dùng)
...
if __name__ == '__main__':
    ...
    qnn_layers = 2
    q_device_name = "default.qubit"  # switch to "lightning.qubit" if installed for speed
    ...
    with open("ModelInfo/model_info_QSAC_6qubits_2layers_desaturation_ver3.txt", "w", encoding="utf-8") as f:
    ...
    for ep in range(episodes):
        ...
        print(f"Episode {ep}, Reward: {total_reward:.3f}, | Demand=({demand_lte:.2f}, {demand_nr :.2f}) "
        ...
        pd.DataFrame([{"episode": ep, "reward": total_reward, "alloc_lte":alloc_lte, "alloc_nr": alloc_nr, "demand_lte":demand_lte, "demand_nr":demand_nr}]).to_csv(
            "logs3/sac_qactor_training_log_ver3.csv",
            mode='a',
            header=not os.path.exists("logs3/sac_qactor_training_log_ver3.csv"),
            index=False
        )
        if (ep + 1) % 2000 == 0:
    ...
    # Save & plot
    torch.save(agent.actor.state_dict(), "Model/sac_qactor_actor_model_morePara_desaturation_ver3.pth")
    pd.DataFrame(logs).to_csv("Result/sac_qactor_training_log_morePara_desaturation_ver3.csv", index=False)
    ...
    plt.grid(True)
    plt.savefig("FigureReward/MorePara_desaturation_sac_qactor_training_reward_plot_ver3.png")
    plt.show()
