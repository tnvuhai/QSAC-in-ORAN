...
class ReUploadingVQC(nn.Module):
    def __init__(self, n_qubits: int, qnn_layers: int, device_name: str = "lightning.gpu"):
        super().__init__()
...
class QuantumActor(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, max_action: float,
                 n_qubits: int = 6, qnn_layers: int = 2, device_name: str = "lightning.gpu"):
        super().__init__()
...
if __name__ == '__main__':
    ...
    qnn_layers = 2
    q_device_name = "lightning.gpu"  # switch to "lightning.qubit" if installed for speed
    ...
    with open("ModelInfo/model_info_QSAC_6qubits_2layers_ver2.txt", "w", encoding="utf-8") as f:
    ...
    for ep in range(episodes):
        ...
        pd.DataFrame([{"episode": ep, "reward": total_reward, "alloc_lte":alloc_lte, "alloc_nr": alloc_nr, "demand_lte":demand_lte, "demand_nr":demand_nr}]).to_csv(
            "logs2/sac_qactor_training_log.csv",
            mode='a',
            header=not os.path.exists("logs2/sac_qactor_training_log.csv"),
            index=False
        )
        if (ep + 1) % 4000 == 0:
            save_checkpoint(agent, buffer, ep, f"Checkpoint2/sac_qsacActor_Test2_ep{ep+1}.pth")
        

    # Save & plot
    torch.save(agent.actor.state_dict(), "Model/sac_qactor_actor_model_morePara_ver2.pth")
    pd.DataFrame(logs).to_csv("Result/sac_qactor_training_log_morePara_ver2.csv", index=False)
    ...
    plt.grid(True)
    plt.savefig("FigureReward/MorePara_ver2_sac_qactor_training_reward_plot.png")
    plt.show()
