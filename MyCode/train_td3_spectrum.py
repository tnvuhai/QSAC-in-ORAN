...
class SpectrumEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        for i in range(self.t - self.history, self.t):
            obs.extend([self.lte[i], self.nr[i]])
    ...
    def step(self, action):
        ...
        D_L, D_N = self.lte[self.t], self.nr[self.t]
...
def evaluate_model(model_path, lte_demand, nr_demand, Nr=60, zeta=0.5, history=3):
    ...
    for i in range(history):
        obs.extend([lte_demand[i], nr_demand[i]])
...
def main():
    ...
    # Plot Learning Curves (optional placeholder)
    # You can use TensorBoard separately for detailed training visualization
