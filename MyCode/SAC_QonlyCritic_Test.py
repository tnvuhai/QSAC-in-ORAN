...
if __name__ == '__main__':
    ...
    for ep in range(episodes):
        ...
        if (ep + 1) % 4000 == 0:
            save_checkpoint(agent, buffer, ep, f"Checkpoint/sac_qsacActor_Test_ep{ep+1}.pth")
        

    # Save & plot
    torch.save(agent.actor.state_dict(), "Model/sac_qactor_actor_model_morePara.pth")
    pd.DataFrame(logs).to_csv("Result/sac_qactor_training_log_morePara.csv", index=False)
    ...
    plt.grid(True)
    plt.savefig("FigureReward/MorePara_sac_qactor_training_reward_plot.png")
    plt.show()
