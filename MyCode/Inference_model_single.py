...
def load_actor_from_checkpoint(path: str, actor: torch.nn.Module, map_location=None):
    """
    Robust loader:
      - Hỗ trợ checkpoint dạng full dict {'actor': state_dict, ...} hoặc trực tiếp state_dict
      - Tự động thử các khoá phổ biến: 'actor', 'state_dict', 'policy'
      - In missing/unexpected keys giúp debug
    """
    ...
    if isinstance(ckpt, dict):
        if "actor" in ckpt and isinstance(ckpt["actor"], (dict,)):
            ...
            # Nếu dict nhưng không rõ, có thể chính là state_dict
            candidate = ckpt
    else:
        candidate = ckpt
    ...
    # Trả về bất kỳ metadata nào có trong ckpt (nếu có) để ghi vào info file
    meta = {}
    if isinstance(ckpt, dict):
...
# ckpt = "Checkpoint/sac_qsacActor_desaturation_ep4000.pth"

arch = "qactor_classic_critic"  # test de2
ckpt = "Checkpoint2/qsac_ver2_qsacActor_desaturation_ep16000.pth"

# arch = "classical"  
# ckpt = "Model/sac_actor_model.pth" # SAC
# ckpt = "Model/td3_actor_model.pth" # TD3
# ckpt = "Model/ddpg_actor_model.pth" # DDPG

device_str = "cuda" if torch.cuda.is_available() else "cpu"
NUM_STEPS = 100
save_plot = "qsac2_metrics_4panel16k.png"
save_csv = "qsac2_metrics_qsac.csv"
dump_model_info = True

# -------------------------------
# Build QSAC Agent
# -------------------------------
def build_qsac_agent(arch: str, state_dim: int, action_dim: float, max_action: float, device: torch.device):
    arch = arch.lower()
    print(arch)
    if arch in {"qactor_classic_critic", "quantum_actor_classic_critic", "qa_cc"}:
        from SAC_QonlyCritic_desaturation2 import SACAgent as QSACAgent
        ...
        return agent, "classical"
        # from _DDPG import DDPGAgent as ClassicalDDPG
        # agent = ClassicalDDPG(state_dim, action_dim, max_action, device)
        # return agent, "classical"
...
# --------- CHỈ THÊM 3 DÒNG BÊN DƯỚI (thay cho 2 dòng load cũ) ---------
ckpt_meta = load_actor_from_checkpoint(ckpt, agent.actor, map_location=device)
# -----------------------------------------------------------------------
...
if dump_model_info:
    ...
    info_path = os.path.join("ModelName", f"inference_qsac2_model_info_{resolved_arch}.txt")
    with open(info_path, "w", encoding="utf-8") as f:
        f.write(f"ARCH: {resolved_arch}\n\n")
        if ckpt_meta:
            f.write("===== CHECKPOINT META =====\n")
...
for step in range(NUM_STEPS):
    ...
    #action = np.clip((action * 0.5 + 0.5), 0.01, 1.0)
