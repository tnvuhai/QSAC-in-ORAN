...
if __name__ == '__main__':
    ...
    qnn_layers = 2
    q_device_name = "default.qubit.torch"  # switch to "lightning.qubit" if installed for speed
    ...
    with open("Model/model_info_Normal_SAC.txt", "w", encoding="utf-8") as f:
    ...
    print(f"Số tham số Actor: {actor_params:,}")
    print(f"Số tham số Critic: {critic_params:,}")
    print(f"Tổng: {actor_params + critic_params:,}")

    quantum_params = sum(p.numel() for name, p in agent.actor.named_parameters()
                     if p.requires_grad and 'mean_head' not in name and 'log_std_head' not in name)

    print(f"Số tham số Quantum trong Actor: {quantum_params:,}")
