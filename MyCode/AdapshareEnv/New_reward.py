...
class ResourceAllocationEnv(gym.Env):
    def __init__(self, lte_file, nr_file, gamma=0.5, N_R=100, history_len=10, scaling=0.01, max_steps=1):
        super().__init__()
        self.gamma = gamma
        self.N_R = N_R
        ...
        self.observation_space = spaces.Box(low=0.0, high=100.0, shape=(2*history_len + 2,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        terminated = self.idx >= self.total_len   # hết dữ liệu
        truncated = self.step_count >= self.max_steps  # bị giới hạn thời gian
...
# Tạo môi trường
env = ResourceAllocationEnv(lte_file=lte_file, nr_file=nr_file, gamma=0.5, N_R=100)
