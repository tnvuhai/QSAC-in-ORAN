...
class ResourceAllocationEnv(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Each episode has only 1 step => reset the step counter,
        # but DO NOT reset self.idx so that time continues to run 0->1->...->n->0->...
        self.step_count = 0

        # Only initialize idx = 0 the very first time
        ...
        # move the cursor for the next episode
    ...
    def step(self, action):
        ...
        # demand at the current time t (before incrementing idx)
        ...
        # advance time: wrap around at the end
        self.idx = (self.idx + 1) % self.total_len
        self.step_count += 1
        #print(self.idx)
        # no longer use terminated based on idx; only truncated based on steps/episode
        terminated = False
        ...
        return self._get_obs(), reward, terminated, truncated, {
            ...
            "t_idx": (self.idx - 1) % self.total_len   # (optional) index just used
        }
...
# def normalize_action(action_raw, low=0.0, high=1.0):
#     return low + 0.5 * (action_raw + 1.0) * (high - low)