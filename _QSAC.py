...
# =============================================================
# 0) Data Re-Uploading and VQC Layer
...
class ReUploadingVQC(nn.Module):
    def __init__(self, n_qubits: int, qnn_layers: int, device_name: str = "lightning.qubit"):
        ...
        def circuit(inputs, ent_w, omega, phi, rot_pre):
            ...
            # Pre-measurement rotations to "bring" the operating point to ~0
            for q in range(self.n_qubits):
                a,b,g = rot_pre[q]
                qml.Rot(a, b, g, wires=q)

            # Multi-axis pooling: Z, X, Y
...
# =============================================================
# 1) Quantum Actor (hybrid): Quantum VQC + small classical heads
# =============================================================
class QuantumActor(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, max_action: float,
        ...
        self.mean_head    = nn.Linear(3 * n_qubits, action_dim)  
...
class QSACAgent:
    ...
    def train(self, replay_buffer, batch_size=48, gamma=1.0, tau=1e-4):
        ...
        if do_policy:
            ...
            # ----- Target update (each step; can also be delayed if desired) -----
...
if __name__ == '__main__':
    ...
    qnn_layers = 2
    q_device_name = "lightning.qubit"  # switch to "lightning.qubit" or "lightning.gpu" if you have installed library for speed. If you don't have a GPU, use "default.qubit"
    ...
    rewards = []
    logs = []
    resume_path = None # You can train from pretrained model if needed, e.g., "Checkpoint/sac_qsacActor.pth"
    if resume_path:
    ...
    for ep in range(episodes):
        ...
        if (ep + 1) % 2000 == 0:
            save_checkpoint(agent, buffer, ep, f"Checkpoint/qsac_qsacActor_ep{ep+1}.pth")
