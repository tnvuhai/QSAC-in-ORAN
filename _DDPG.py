...
class DDPGAgent:
    ...
    def select_action(self, state, evaluate=False):
        ...
        action_raw = self.actor(state_tensor).cpu().data.numpy().flatten()
    ...
    def train(self, replay_buffer, batch_size):
        ...
        with torch.no_grad():
            # Gaussian noise for target action (policy smoothing)
            noise = (torch.randn_like(a) * self.target_policy_noise).clamp(-self.noise_clip, self.noise_clip)
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            # soft update both actor + critic target
            for param, target_param in zip(self.critic.parameters(), self.critic_target.parameters()):
                target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)
            for param, target_param in zip(self.actor.parameters(), self.actor_target.parameters()):
                target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)
        else:
            # only soft-update critic target to keep pace
            for param, target_param in zip(self.critic.parameters(), self.critic_target.parameters()):
                target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)
