...
OUTPUT_DIR = "Inference_output"  # Folder to save outputs
...
def compute_metrics_from_timeseries(df: pd.DataFrame, N_R: float = 60.0,
    ...
    for m, sub in df.groupby("model"):
        ...
        # --- ϵ−band compliance rate
        ob = {}
        for eps in eps_list:
            ob[f"ϵ−band compliance rate_lte_{int(eps*100)}%"] = np.mean(
                np.abs(sub["alloc_lte"].values - sub["demand_lte"].values)
                <= eps * np.maximum(sub["demand_lte"].values, 1e-6)
            )
            ob[f"ϵ−band compliance rate_nr_{int(eps*100)}%"] = np.mean(
...
# 1–4) Allocation vs Demand for each model
for m in model_order:
    ...
    out_png = os.path.join(OUTPUT_DIR, f"alloc_vs_demand_{m}.pdf")
    fig.tight_layout()
    fig.savefig(out_png, dpi=300)
    print(f"[INFO] saved -> {out_png}")
    plt.close(fig)

# 5) Surplus/Deficit LTE over time
fig, ax = plt.subplots(figsize=(7, 4))
for m in model_order:
    sub = df[df["model"] == m].sort_values("step")
    ax.plot(sub["step"].values, sub["surplus_lte"].values, linestyle="--", label=m)
#ax.set_title("Surplus/Deficit over Time (LTE)", fontsize=17)
ax.set_xlabel("Step", fontsize=17); ax.set_ylabel("Surplus/Deficit", fontsize=17)
ax.tick_params(axis='x', labelsize=17)
ax.tick_params(axis='y', labelsize=17)
ax.grid(True, alpha=0.3); ax.legend(loc="best", fontsize=12) 
out_png = os.path.join(OUTPUT_DIR, "surplus_lte.pdf")
fig.tight_layout(); fig.savefig(out_png, dpi=300)
print(f"[INFO] saved -> {out_png}")
plt.close(fig)

# 6) Surplus/Deficit NR over time
fig, ax = plt.subplots(figsize=(7, 4))
for m in model_order:
    sub = df[df["model"] == m].sort_values("step")
    ax.plot(sub["step"].values, sub["surplus_nr"].values, linestyle="-", label=m)
#ax.set_title("Surplus/Deficit over Time (NR)", fontsize=17)
ax.set_xlabel("Step", fontsize=17); ax.set_ylabel("Surplus/Deficit", fontsize=17)
ax.tick_params(axis='x', labelsize=17)
ax.tick_params(axis='y', labelsize=17)
ax.grid(True, alpha=0.3); ax.legend(loc="best", fontsize=12) 
out_png = os.path.join(OUTPUT_DIR, "surplus_nr.pdf")
fig.tight_layout(); fig.savefig(out_png, dpi=300)
print(f"[INFO] saved -> {out_png}")
plt.close(fig)